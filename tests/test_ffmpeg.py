...
def fake_completed(stdout='{}', stderr='', returncode=0):
    return CompletedProcess(args=['ffprobe'], returncode=returncode, stdout=stdout, stderr=stderr)


def test_get_file_info_success(monkeypatch):
    monkeypatch.setattr(ffmpeg, 'run', lambda cmd, capture_output, text: fake_completed('{"format": {"duration": "1.0"}}'))
    info = ffmpeg.get_file_info('/tmp/foo.flac')
    assert 'format' in info


def test_get_file_info_failure(monkeypatch):
    monkeypatch.setattr(ffmpeg, 'run', lambda cmd, capture_output, text: fake_completed('', 'error', returncode=1))
    try:
...
def test_flac_to_wav_success(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ffmpeg, 'run', lambda cmd, capture_output, text: fake_completed())
...
def test_flac_to_wav_failure_removes_partial(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(ffmpeg, 'run', lambda cmd, capture_output, text: fake_completed('', 'err', returncode=1))
    in_f = tmp_path / 'in.flac'
    out_f = tmp_path / 'out.wav'
    in_f.write_bytes(b'fake')
